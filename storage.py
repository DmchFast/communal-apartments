...
def load_user_meters(username: str) -> dict[int, dict]:
...
def save_user_meters(username: str, meters: dict[int, dict]) -> None:
    all_data = load_json(METERS_FILE) or {}
...
def load_user_readings(username: str) -> list[dict]:
...
def save_user_readings(username: str, readings: list[dict]) -> None:
    all_data = load_json(READINGS_FILE) or {}
